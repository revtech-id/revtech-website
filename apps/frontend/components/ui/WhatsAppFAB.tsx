    ...
    <div ref={fabRef} className="fixed bottom-6 right-6 z-50">
      ...
      <div className="absolute bottom-full right-0 mb-5 flex flex-col gap-4">
        ...
        <div className={`relative flex items-center justify-end group origin-bottom transition-all duration-300 ease-out ${isMenuOpen ? "scale-100 opacity-100 translate-y-0 delay-100" : "scale-50 opacity-0 translate-y-8 pointer-events-none"}`}>
          <span className="absolute right-[60px] bg-gray-900 text-white text-[13px] font-semibold px-3.5 py-1.5 rounded-lg whitespace-nowrap shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
            Tanya AI
          </span>
          <button 
            onClick={openChatbot}
            className="flex items-center justify-center w-[50px] h-[50px] bg-blue-600 rounded-full shadow-md hover:shadow-xl hover:scale-110 transition-all duration-300 relative z-10 border border-blue-400/50"
            aria-label="Tanya AI"
          >
            <img 
        ...
        <div className={`relative flex items-center justify-end group origin-bottom transition-all duration-300 ease-out ${isMenuOpen ? "scale-100 opacity-100 translate-y-0 delay-75" : "scale-50 opacity-0 translate-y-8 pointer-events-none"}`}>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center justify-center w-[50px] h-[50px] bg-[#25D366] text-white rounded-full shadow-md hover:shadow-xl hover:scale-110 transition-all duration-300 relative z-10"
            aria-label="Konsultasi WhatsApp"
            onClick={() => setIsMenuOpen(false)}
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="w-[28px] h-[28px]" fill="currentColor" viewBox="0 0 24 24">
      ...
      <button
        ...
        className={`flex items-center justify-center w-[64px] h-[64px] rounded-full shadow-2xl transition-all duration-300 ${isHighlighting ? "ring-8 ring-blue-400/60 scale-110" : ""} ${isMenuOpen ? "bg-gray-900 text-white rotate-45 scale-90" : "bg-blue-600 text-white hover:bg-blue-700 hover:scale-105"}`}
