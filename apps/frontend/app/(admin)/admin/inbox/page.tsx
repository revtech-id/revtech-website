...
import { Pencil, Trash2, MessageSquare, Handshake, X, ChevronDown, Globe, MonitorPlay, Box, SlidersHorizontal, CheckCircle2 } from "lucide-react";
...
function DealModal({ lead, onConfirm, onClose }: DealModalProps) {
  ...
  const [dp, setDp] = useState(Math.round(defaultTotal / 2));
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-[var(--adm-card)] border border-[var(--adm-border)] rounded-2xl shadow-2xl w-full max-w-md p-6"
        ...
        >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setTotal(v);
                    setDp(Math.round(v / 2));
                  }}
                  className="w-full px-3 py-2 rounded-lg border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                />
              </div>
              <div>
          ...
          <div className="flex gap-2 mt-6">
            <button
              onClick={onClose}
              className="flex-1 py-2 rounded-lg font-semibold text-[var(--adm-text-2)] hover:bg-[var(--adm-bg)] hover:text-[var(--adm-text)] transition-colors text-sm border border-[var(--adm-border)]"
            >
              Batal
            </button>
            <button
              onClick={() => onConfirm({ total, dp, deadline, handover })}
              disabled={!total || !dp}
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        {view === "list" ? (
          <>
          <div className="flex items-center flex-1 max-w-full sm:max-w-[460px] rounded-full border bg-[var(--adm-card)] shadow-sm transition-all focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500" style={{ borderColor: "var(--adm-border)" }}>
            {/* Dropdown Layanan */}
      ...
      {view === "list" && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {/* Tabs & Actions Row */}
          <div className="flex flex-col sm:flex-row sm:items-end justify-between border-b border-[var(--adm-border)] mb-6 gap-4 sm:gap-0">
            ...
            <div className="flex items-center pb-2.5 shrink-0 self-start sm:self-auto px-1 sm:px-0">
              <div className="relative flex items-center justify-center shrink-0">
                <button className="text-[var(--adm-text-3)] hover:text-[var(--adm-text)] transition-colors focus:outline-none">
                      ...
                      <div className="p-4 flex flex-col gap-2">
                        ...
                        <div className="flex flex-wrap justify-between items-start gap-4">
                          <div className="flex flex-wrap items-center gap-2 text-[11px] text-[var(--adm-text-2)] font-medium mt-1">
                            ...
                            {lead.company && (
                              <span className="px-2 py-0.5 rounded bg-[var(--adm-bg)] text-[var(--adm-text-2)] border border-[var(--adm-border)] text-[10px] font-semibold hidden sm:inline-block">
                                {lead.company}
                              </span>
                            )}
                            {(() => {
                        ...
                        <div className="flex flex-wrap items-end justify-between gap-4 mt-1">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5">
                            {lead.budget && lead.budget !== "-" && (
                              <span className="px-2 py-0.5 rounded bg-[var(--adm-bg)] text-[var(--adm-text-2)] border border-[var(--adm-border)] text-[10px] font-semibold">
                                {lead.budget}
                              </span>
                            )}
                            {lead.referenceLink && (
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              {deletingId === lead.id ? (
                                <div className="flex items-center gap-1">
                                  <button onClick={() => handleDelete(lead.id)} className="px-2 py-1 text-[10px] font-bold bg-[var(--adm-danger)] text-white rounded hover:opacity-90 transition-opacity">Hapus</button>
                                  <button onClick={() => setDeletingId(null)} className="px-2 py-1 text-[10px] bg-[var(--adm-card)] text-[var(--adm-text-2)] rounded border border-[var(--adm-border)]">Batal</button>
                                </div>
                              ) : (
                                <button
              ...
              <form onSubmit={handleAddLead} className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input required type="text" value={newLead.name} onChange={e => setNewLead({...newLead, name: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)]" placeholder="Masukkan nama lengkap" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold mb-1.5 block text-[var(--adm-text-2)]">Nomor WhatsApp *</label>
                    <div className="flex border border-[var(--adm-border)] rounded-lg bg-transparent focus-within:ring-2 focus-within:ring-[var(--adm-accent)]/30 focus-within:border-[var(--adm-accent)] transition-all">
                      <CountrySelector 
                        selected={selectedCountry} 
                        onSelect={setSelectedCountry} 
                        theme="admin" 
                      />
                      <input required type="text" value={newLead.phone} onChange={e => setNewLead({...newLead, phone: e.target.value.replace(/\D/g,'')})} className="w-full px-3 py-2.5 text-sm bg-transparent border-0 text-[var(--adm-text)] focus:outline-none focus:ring-0 placeholder-[var(--adm-text-3)]" placeholder="8123456..." />
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input type="text" value={newLead.company} onChange={e => setNewLead({...newLead, company: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)]" placeholder="Masukkan nama bisnis atau instansi" />
                </div>
                ...
                <div className={`grid grid-cols-1 ${newLead.service === "Custom Project" ? "" : "md:grid-cols-2"} gap-5`}>
                  <div>
                    ...
                    <select value={newLead.service} onChange={e => setNewLead({...newLead, service: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)]">
                  ...
                  {newLead.service === "Jasa Website" ? (
                    <div>
                      ...
                      <select value={newLead.serviceDetail} onChange={e => setNewLead({...newLead, serviceDetail: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)]">
                    ...
                    <div>
                      ...
                      <input type="text" value={newLead.serviceDetail} onChange={e => setNewLead({...newLead, serviceDetail: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)]" placeholder="Masukkan nama produk" />
                    </div>
                  ) : null}
                </div>

                {newLead.service === "Jasa Website" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 p-5 bg-[var(--adm-bg)] rounded-xl border border-[var(--adm-border)]">
                    <div>
                      <label className="text-xs font-semibold mb-1.5 block text-[var(--adm-text-2)]">Opsi Serah Terima</label>
                      <select value={newLead.handover} onChange={e => setNewLead({...newLead, handover: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-[var(--adm-card)] text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)]">
                    ...
                    <div className="flex items-start sm:items-center gap-3 md:pt-[22px]">
                      <input 
                        ...
                        onChange={e => setNewLead({...newLead, isVip: e.target.checked})}
                        className="w-[18px] h-[18px] mt-0.5 sm:mt-0 rounded border-[var(--adm-border)] bg-[var(--adm-card)] text-[var(--adm-accent)] focus:ring-[var(--adm-accent)] cursor-pointer shrink-0"
                      />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <div className="flex border border-[var(--adm-border)] rounded-lg bg-transparent focus-within:ring-2 focus-within:ring-[var(--adm-accent)]/30 focus-within:border-[var(--adm-accent)] transition-all">
                      <div className="bg-transparent border-r border-[var(--adm-border)] rounded-l-lg text-[var(--adm-text)] font-bold text-[13px] px-4 flex items-center justify-center pointer-events-none">
                        Rp
                      </div>
                      <input 
                        type="text" 
                  ...
                  <div>
                    ...
                    <select value={newLead.status} onChange={e => setNewLead({...newLead, status: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)]">
                      <option value="new" className="bg-[var(--adm-card)] text-[var(--adm-text)]">Baru Masuk</option>
                      <option value="followup" className="bg-[var(--adm-card)] text-[var(--adm-text)]">Tindak Lanjut</option>
                      <option value="waiting_dp" className="bg-[var(--adm-card)] text-[var(--adm-text)]">Menunggu DP</option>
                      <option value="ghosting" className="bg-[var(--adm-card)] text-[var(--adm-text)]">Batal</option>
                ...
                {newLead.service === "Custom Project" && (
                  <div>
                    ...
                    <input type="text" value={newLead.referenceLink} onChange={e => setNewLead({...newLead, referenceLink: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)]" placeholder="cth: www.contoh.com" />
                  </div>
                )}

                <div>
                  <label className="text-xs font-semibold mb-1.5 block text-[var(--adm-text-2)]">Pesan / Kebutuhan Klien</label>
                  <textarea rows={3} value={newLead.message} onChange={e => setNewLead({...newLead, message: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)] resize-none" placeholder={newLead.service === "Custom Project" ? "Ceritakan ide sistem, web app, atau solusi custom yang Anda butuhkan secara singkat..." : "Tuliskan kebutuhan spesifik dari klien..."} />
                </div>

                <div>
                  <label className="text-xs font-semibold mb-1.5 block text-[var(--adm-text-2)]">Catatan Follow-up Internal (hanya admin)</label>
                  <input type="text" value={newLead.followUpNote} onChange={e => setNewLead({...newLead, followUpNote: e.target.value})} className="w-full px-3 py-2.5 rounded-lg text-sm border border-[var(--adm-border)] bg-transparent text-[var(--adm-text)] focus:outline-none focus:ring-2 focus:ring-[var(--adm-accent)]/30 focus:border-[var(--adm-accent)]" placeholder="Cth: Klien mau diingatkan lagi Senin depan" />
                </div>

                <div className="pt-2 flex items-center justify-end gap-3">
                  <button type="button" onClick={() => setView("list")} className="px-5 py-2 rounded-lg font-semibold text-[var(--adm-text-2)] border border-[var(--adm-border)] hover:bg-[var(--adm-bg)] hover:text-[var(--adm-text)] transition-colors text-sm">Batal</button>
